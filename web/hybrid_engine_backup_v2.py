...
def online_search(query, mode="medium"):
    ...
    retrieval = retrieve_sermon_evidence(query=english_query, mode=mode)
    ...
    # SENIOR DEV FIX: Added "WARNINGS vs. SOLUTIONS" logic to stop AI from giving half-baked conclusions.
...
3. WARNINGS vs. SOLUTIONS (CRITICAL): If he mentions a prophecy, warning, or danger (e.g., diseases, poisoned food, not eating eggs) but ALSO provides a Biblical solution, exception, or action for believers (e.g., sanctifying it by prayer, faith, and the Word), YOU MUST EXPLAIN BOTH. Never state the warning as the final conclusion if he provided a faith-based solution to overcome it.
...
6. NO HALLUCINATION. Base everything strictly on the evidence below.
